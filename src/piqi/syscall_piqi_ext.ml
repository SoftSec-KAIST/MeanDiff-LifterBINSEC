...
let print_syscall_pol ?opts x =
  Pervasives.print_endline (gen_syscall_pol x `piq ?opts)
let prerr_syscall_pol ?opts x =
  Pervasives.prerr_endline (gen_syscall_pol x `piq ?opts)

let print_syscall_t ?opts x =
  Pervasives.print_endline (gen_syscall_t x `piq ?opts)
let prerr_syscall_t ?opts x =
  Pervasives.prerr_endline (gen_syscall_t x `piq ?opts)

let print_nr_read_pol ?opts x =
  Pervasives.print_endline (gen_nr_read_pol x `piq ?opts)
let prerr_nr_read_pol ?opts x =
  Pervasives.prerr_endline (gen_nr_read_pol x `piq ?opts)

let print_nr_open_t ?opts x =
  Pervasives.print_endline (gen_nr_open_t x `piq ?opts)
let prerr_nr_open_t ?opts x =
  Pervasives.prerr_endline (gen_nr_open_t x `piq ?opts)

let print_nr_read_t ?opts x =
  Pervasives.print_endline (gen_nr_read_t x `piq ?opts)
let prerr_nr_read_t ?opts x =
  Pervasives.prerr_endline (gen_nr_read_t x `piq ?opts)

let print_nr_write_t ?opts x =
  Pervasives.print_endline (gen_nr_write_t x `piq ?opts)
let prerr_nr_write_t ?opts x =
  Pervasives.prerr_endline (gen_nr_write_t x `piq ?opts)

let print_sys_generic_t ?opts x =
  Pervasives.print_endline (gen_sys_generic_t x `piq ?opts)
let prerr_sys_generic_t ?opts x =
  Pervasives.prerr_endline (gen_sys_generic_t x `piq ?opts)

let print_syscall_ident ?opts x =
  Pervasives.print_endline (gen_syscall_ident x `piq ?opts)
let prerr_syscall_ident ?opts x =
  Pervasives.prerr_endline (gen_syscall_ident x `piq ?opts)
