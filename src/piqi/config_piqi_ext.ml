...
let print_call_name_t ?opts x =
  Pervasives.print_endline (gen_call_name_t x `piq ?opts)
let prerr_call_name_t ?opts x =
  Pervasives.prerr_endline (gen_call_name_t x `piq ?opts)

let print_configuration ?opts x =
  Pervasives.print_endline (gen_configuration x `piq ?opts)
let prerr_configuration ?opts x =
  Pervasives.prerr_endline (gen_configuration x `piq ?opts)

let print_input_t ?opts x =
  Pervasives.print_endline (gen_input_t x `piq ?opts)
let prerr_input_t ?opts x =
  Pervasives.prerr_endline (gen_input_t x `piq ?opts)

let print_input_t_input_kind ?opts x =
  Pervasives.print_endline (gen_input_t_input_kind x `piq ?opts)
let prerr_input_t_input_kind ?opts x =
  Pervasives.prerr_endline (gen_input_t_input_kind x `piq ?opts)

let print_input_t_when_t ?opts x =
  Pervasives.print_endline (gen_input_t_when_t x `piq ?opts)
let prerr_input_t_when_t ?opts x =
  Pervasives.prerr_endline (gen_input_t_when_t x `piq ?opts)
