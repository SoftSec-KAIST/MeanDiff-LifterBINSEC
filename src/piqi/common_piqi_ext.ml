...
let print_register_value_t ?opts x =
  Pervasives.print_endline (gen_register_value_t x `piq ?opts)
let prerr_register_value_t ?opts x =
  Pervasives.prerr_endline (gen_register_value_t x `piq ?opts)

let print_register_t ?opts x =
  Pervasives.print_endline (gen_register_t x `piq ?opts)
let prerr_register_t ?opts x =
  Pervasives.prerr_endline (gen_register_t x `piq ?opts)

let print_memory_t ?opts x =
  Pervasives.print_endline (gen_memory_t x `piq ?opts)
let prerr_memory_t ?opts x =
  Pervasives.prerr_endline (gen_memory_t x `piq ?opts)

let print_indirect_register_t ?opts x =
  Pervasives.print_endline (gen_indirect_register_t x `piq ?opts)
let prerr_indirect_register_t ?opts x =
  Pervasives.prerr_endline (gen_indirect_register_t x `piq ?opts)

let print_memory_pol ?opts x =
  Pervasives.print_endline (gen_memory_pol x `piq ?opts)
let prerr_memory_pol ?opts x =
  Pervasives.prerr_endline (gen_memory_pol x `piq ?opts)

let print_smt_result ?opts x =
  Pervasives.print_endline (gen_smt_result x `piq ?opts)
let prerr_smt_result ?opts x =
  Pervasives.prerr_endline (gen_smt_result x `piq ?opts)

let print_tracing_action ?opts x =
  Pervasives.print_endline (gen_tracing_action x `piq ?opts)
let prerr_tracing_action ?opts x =
  Pervasives.prerr_endline (gen_tracing_action x `piq ?opts)

let print_action ?opts x =
  Pervasives.print_endline (gen_action x `piq ?opts)
let prerr_action ?opts x =
  Pervasives.prerr_endline (gen_action x `piq ?opts)

let print_call_convention_t ?opts x =
  Pervasives.print_endline (gen_call_convention_t x `piq ?opts)
let prerr_call_convention_t ?opts x =
  Pervasives.prerr_endline (gen_call_convention_t x `piq ?opts)

let print_ir_kind_t ?opts x =
  Pervasives.print_endline (gen_ir_kind_t x `piq ?opts)
let prerr_ir_kind_t ?opts x =
  Pervasives.prerr_endline (gen_ir_kind_t x `piq ?opts)

let print_solver_t ?opts x =
  Pervasives.print_endline (gen_solver_t x `piq ?opts)
let prerr_solver_t ?opts x =
  Pervasives.prerr_endline (gen_solver_t x `piq ?opts)

let print_analysis_direction_t ?opts x =
  Pervasives.print_endline (gen_analysis_direction_t x `piq ?opts)
let prerr_analysis_direction_t ?opts x =
  Pervasives.prerr_endline (gen_analysis_direction_t x `piq ?opts)

let print_proto_size_t ?opts x =
  Pervasives.print_endline (gen_proto_size_t x `piq ?opts)
let prerr_proto_size_t ?opts x =
  Pervasives.prerr_endline (gen_proto_size_t x `piq ?opts)
