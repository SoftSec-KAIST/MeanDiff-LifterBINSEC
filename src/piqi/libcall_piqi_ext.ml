...
let print_libcall_pol ?opts x =
  Pervasives.print_endline (gen_libcall_pol x `piq ?opts)
let prerr_libcall_pol ?opts x =
  Pervasives.prerr_endline (gen_libcall_pol x `piq ?opts)

let print_libcall_t ?opts x =
  Pervasives.print_endline (gen_libcall_t x `piq ?opts)
let prerr_libcall_t ?opts x =
  Pervasives.prerr_endline (gen_libcall_t x `piq ?opts)

let print_generic_pol ?opts x =
  Pervasives.print_endline (gen_generic_pol x `piq ?opts)
let prerr_generic_pol ?opts x =
  Pervasives.prerr_endline (gen_generic_pol x `piq ?opts)

let print_ctype_b_loc_pol ?opts x =
  Pervasives.print_endline (gen_ctype_b_loc_pol x `piq ?opts)
let prerr_ctype_b_loc_pol ?opts x =
  Pervasives.prerr_endline (gen_ctype_b_loc_pol x `piq ?opts)

let print_exit_pol ?opts x =
  Pervasives.print_endline (gen_exit_pol x `piq ?opts)
let prerr_exit_pol ?opts x =
  Pervasives.prerr_endline (gen_exit_pol x `piq ?opts)

let print_fscanf_pol ?opts x =
  Pervasives.print_endline (gen_fscanf_pol x `piq ?opts)
let prerr_fscanf_pol ?opts x =
  Pervasives.prerr_endline (gen_fscanf_pol x `piq ?opts)

let print_read_pol ?opts x =
  Pervasives.print_endline (gen_read_pol x `piq ?opts)
let prerr_read_pol ?opts x =
  Pervasives.prerr_endline (gen_read_pol x `piq ?opts)

let print_fread_pol ?opts x =
  Pervasives.print_endline (gen_fread_pol x `piq ?opts)
let prerr_fread_pol ?opts x =
  Pervasives.prerr_endline (gen_fread_pol x `piq ?opts)

let print_strcmp_pol ?opts x =
  Pervasives.print_endline (gen_strcmp_pol x `piq ?opts)
let prerr_strcmp_pol ?opts x =
  Pervasives.prerr_endline (gen_strcmp_pol x `piq ?opts)

let print_strncmp_pol ?opts x =
  Pervasives.print_endline (gen_strncmp_pol x `piq ?opts)
let prerr_strncmp_pol ?opts x =
  Pervasives.prerr_endline (gen_strncmp_pol x `piq ?opts)

let print_memcmp_pol ?opts x =
  Pervasives.print_endline (gen_memcmp_pol x `piq ?opts)
let prerr_memcmp_pol ?opts x =
  Pervasives.prerr_endline (gen_memcmp_pol x `piq ?opts)

let print_strchr_pol ?opts x =
  Pervasives.print_endline (gen_strchr_pol x `piq ?opts)
let prerr_strchr_pol ?opts x =
  Pervasives.prerr_endline (gen_strchr_pol x `piq ?opts)

let print_strncpy_pol ?opts x =
  Pervasives.print_endline (gen_strncpy_pol x `piq ?opts)
let prerr_strncpy_pol ?opts x =
  Pervasives.prerr_endline (gen_strncpy_pol x `piq ?opts)

let print_fstat_pol ?opts x =
  Pervasives.print_endline (gen_fstat_pol x `piq ?opts)
let prerr_fstat_pol ?opts x =
  Pervasives.prerr_endline (gen_fstat_pol x `piq ?opts)

let print_fxstat64_pol ?opts x =
  Pervasives.print_endline (gen_fxstat64_pol x `piq ?opts)
let prerr_fxstat64_pol ?opts x =
  Pervasives.prerr_endline (gen_fxstat64_pol x `piq ?opts)

let print_realloc_pol ?opts x =
  Pervasives.print_endline (gen_realloc_pol x `piq ?opts)
let prerr_realloc_pol ?opts x =
  Pervasives.prerr_endline (gen_realloc_pol x `piq ?opts)

let print_mmap_pol ?opts x =
  Pervasives.print_endline (gen_mmap_pol x `piq ?opts)
let prerr_mmap_pol ?opts x =
  Pervasives.prerr_endline (gen_mmap_pol x `piq ?opts)

let print_qsort_pol ?opts x =
  Pervasives.print_endline (gen_qsort_pol x `piq ?opts)
let prerr_qsort_pol ?opts x =
  Pervasives.prerr_endline (gen_qsort_pol x `piq ?opts)

let print_bsearch_pol ?opts x =
  Pervasives.print_endline (gen_bsearch_pol x `piq ?opts)
let prerr_bsearch_pol ?opts x =
  Pervasives.prerr_endline (gen_bsearch_pol x `piq ?opts)

let print_open_pol ?opts x =
  Pervasives.print_endline (gen_open_pol x `piq ?opts)
let prerr_open_pol ?opts x =
  Pervasives.prerr_endline (gen_open_pol x `piq ?opts)

let print_strcpy_pol ?opts x =
  Pervasives.print_endline (gen_strcpy_pol x `piq ?opts)
let prerr_strcpy_pol ?opts x =
  Pervasives.prerr_endline (gen_strcpy_pol x `piq ?opts)

let print_printf_pol ?opts x =
  Pervasives.print_endline (gen_printf_pol x `piq ?opts)
let prerr_printf_pol ?opts x =
  Pervasives.prerr_endline (gen_printf_pol x `piq ?opts)

let print_atoi_pol ?opts x =
  Pervasives.print_endline (gen_atoi_pol x `piq ?opts)
let prerr_atoi_pol ?opts x =
  Pervasives.prerr_endline (gen_atoi_pol x `piq ?opts)

let print_malloc_pol ?opts x =
  Pervasives.print_endline (gen_malloc_pol x `piq ?opts)
let prerr_malloc_pol ?opts x =
  Pervasives.prerr_endline (gen_malloc_pol x `piq ?opts)

let print_getmodulehandle_pol ?opts x =
  Pervasives.print_endline (gen_getmodulehandle_pol x `piq ?opts)
let prerr_getmodulehandle_pol ?opts x =
  Pervasives.prerr_endline (gen_getmodulehandle_pol x `piq ?opts)

let print_getprocaddress_pol ?opts x =
  Pervasives.print_endline (gen_getprocaddress_pol x `piq ?opts)
let prerr_getprocaddress_pol ?opts x =
  Pervasives.prerr_endline (gen_getprocaddress_pol x `piq ?opts)

let print_getmainargs_pol ?opts x =
  Pervasives.print_endline (gen_getmainargs_pol x `piq ?opts)
let prerr_getmainargs_pol ?opts x =
  Pervasives.prerr_endline (gen_getmainargs_pol x `piq ?opts)

let print_gethostname_pol ?opts x =
  Pervasives.print_endline (gen_gethostname_pol x `piq ?opts)
let prerr_gethostname_pol ?opts x =
  Pervasives.prerr_endline (gen_gethostname_pol x `piq ?opts)

let print_free_pol ?opts x =
  Pervasives.print_endline (gen_free_pol x `piq ?opts)
let prerr_free_pol ?opts x =
  Pervasives.prerr_endline (gen_free_pol x `piq ?opts)

let print_memcpy_pol ?opts x =
  Pervasives.print_endline (gen_memcpy_pol x `piq ?opts)
let prerr_memcpy_pol ?opts x =
  Pervasives.prerr_endline (gen_memcpy_pol x `piq ?opts)

let print_memset_pol ?opts x =
  Pervasives.print_endline (gen_memset_pol x `piq ?opts)
let prerr_memset_pol ?opts x =
  Pervasives.prerr_endline (gen_memset_pol x `piq ?opts)

let print_fgetc_pol ?opts x =
  Pervasives.print_endline (gen_fgetc_pol x `piq ?opts)
let prerr_fgetc_pol ?opts x =
  Pervasives.prerr_endline (gen_fgetc_pol x `piq ?opts)

let print_lseek_pol ?opts x =
  Pervasives.print_endline (gen_lseek_pol x `piq ?opts)
let prerr_lseek_pol ?opts x =
  Pervasives.prerr_endline (gen_lseek_pol x `piq ?opts)

let print_exit_t ?opts x =
  Pervasives.print_endline (gen_exit_t x `piq ?opts)
let prerr_exit_t ?opts x =
  Pervasives.prerr_endline (gen_exit_t x `piq ?opts)

let print_generic_t ?opts x =
  Pervasives.print_endline (gen_generic_t x `piq ?opts)
let prerr_generic_t ?opts x =
  Pervasives.prerr_endline (gen_generic_t x `piq ?opts)

let print_fscanf_elem_t ?opts x =
  Pervasives.print_endline (gen_fscanf_elem_t x `piq ?opts)
let prerr_fscanf_elem_t ?opts x =
  Pervasives.prerr_endline (gen_fscanf_elem_t x `piq ?opts)

let print_fscanf_t ?opts x =
  Pervasives.print_endline (gen_fscanf_t x `piq ?opts)
let prerr_fscanf_t ?opts x =
  Pervasives.prerr_endline (gen_fscanf_t x `piq ?opts)

let print_ctype_b_loc_t ?opts x =
  Pervasives.print_endline (gen_ctype_b_loc_t x `piq ?opts)
let prerr_ctype_b_loc_t ?opts x =
  Pervasives.prerr_endline (gen_ctype_b_loc_t x `piq ?opts)

let print_read_t ?opts x =
  Pervasives.print_endline (gen_read_t x `piq ?opts)
let prerr_read_t ?opts x =
  Pervasives.prerr_endline (gen_read_t x `piq ?opts)

let print_fread_t ?opts x =
  Pervasives.print_endline (gen_fread_t x `piq ?opts)
let prerr_fread_t ?opts x =
  Pervasives.prerr_endline (gen_fread_t x `piq ?opts)

let print_strchr_t ?opts x =
  Pervasives.print_endline (gen_strchr_t x `piq ?opts)
let prerr_strchr_t ?opts x =
  Pervasives.prerr_endline (gen_strchr_t x `piq ?opts)

let print_strncpy_t ?opts x =
  Pervasives.print_endline (gen_strncpy_t x `piq ?opts)
let prerr_strncpy_t ?opts x =
  Pervasives.prerr_endline (gen_strncpy_t x `piq ?opts)

let print_fstat_t ?opts x =
  Pervasives.print_endline (gen_fstat_t x `piq ?opts)
let prerr_fstat_t ?opts x =
  Pervasives.prerr_endline (gen_fstat_t x `piq ?opts)

let print_fxstat64_t ?opts x =
  Pervasives.print_endline (gen_fxstat64_t x `piq ?opts)
let prerr_fxstat64_t ?opts x =
  Pervasives.prerr_endline (gen_fxstat64_t x `piq ?opts)

let print_realloc_t ?opts x =
  Pervasives.print_endline (gen_realloc_t x `piq ?opts)
let prerr_realloc_t ?opts x =
  Pervasives.prerr_endline (gen_realloc_t x `piq ?opts)

let print_mmap_t ?opts x =
  Pervasives.print_endline (gen_mmap_t x `piq ?opts)
let prerr_mmap_t ?opts x =
  Pervasives.prerr_endline (gen_mmap_t x `piq ?opts)

let print_qsort_t ?opts x =
  Pervasives.print_endline (gen_qsort_t x `piq ?opts)
let prerr_qsort_t ?opts x =
  Pervasives.prerr_endline (gen_qsort_t x `piq ?opts)

let print_bsearch_t ?opts x =
  Pervasives.print_endline (gen_bsearch_t x `piq ?opts)
let prerr_bsearch_t ?opts x =
  Pervasives.prerr_endline (gen_bsearch_t x `piq ?opts)

let print_open_t ?opts x =
  Pervasives.print_endline (gen_open_t x `piq ?opts)
let prerr_open_t ?opts x =
  Pervasives.prerr_endline (gen_open_t x `piq ?opts)

let print_strcpy_t ?opts x =
  Pervasives.print_endline (gen_strcpy_t x `piq ?opts)
let prerr_strcpy_t ?opts x =
  Pervasives.prerr_endline (gen_strcpy_t x `piq ?opts)

let print_strcmp_t ?opts x =
  Pervasives.print_endline (gen_strcmp_t x `piq ?opts)
let prerr_strcmp_t ?opts x =
  Pervasives.prerr_endline (gen_strcmp_t x `piq ?opts)

let print_strncmp_t ?opts x =
  Pervasives.print_endline (gen_strncmp_t x `piq ?opts)
let prerr_strncmp_t ?opts x =
  Pervasives.prerr_endline (gen_strncmp_t x `piq ?opts)

let print_memcmp_t ?opts x =
  Pervasives.print_endline (gen_memcmp_t x `piq ?opts)
let prerr_memcmp_t ?opts x =
  Pervasives.prerr_endline (gen_memcmp_t x `piq ?opts)

let print_printf_t ?opts x =
  Pervasives.print_endline (gen_printf_t x `piq ?opts)
let prerr_printf_t ?opts x =
  Pervasives.prerr_endline (gen_printf_t x `piq ?opts)

let print_atoi_t ?opts x =
  Pervasives.print_endline (gen_atoi_t x `piq ?opts)
let prerr_atoi_t ?opts x =
  Pervasives.prerr_endline (gen_atoi_t x `piq ?opts)

let print_malloc_t ?opts x =
  Pervasives.print_endline (gen_malloc_t x `piq ?opts)
let prerr_malloc_t ?opts x =
  Pervasives.prerr_endline (gen_malloc_t x `piq ?opts)

let print_getmodulehandle_t ?opts x =
  Pervasives.print_endline (gen_getmodulehandle_t x `piq ?opts)
let prerr_getmodulehandle_t ?opts x =
  Pervasives.prerr_endline (gen_getmodulehandle_t x `piq ?opts)

let print_getprocaddress_t ?opts x =
  Pervasives.print_endline (gen_getprocaddress_t x `piq ?opts)
let prerr_getprocaddress_t ?opts x =
  Pervasives.prerr_endline (gen_getprocaddress_t x `piq ?opts)

let print_getmainargs_t ?opts x =
  Pervasives.print_endline (gen_getmainargs_t x `piq ?opts)
let prerr_getmainargs_t ?opts x =
  Pervasives.prerr_endline (gen_getmainargs_t x `piq ?opts)

let print_gethostname_t ?opts x =
  Pervasives.print_endline (gen_gethostname_t x `piq ?opts)
let prerr_gethostname_t ?opts x =
  Pervasives.prerr_endline (gen_gethostname_t x `piq ?opts)

let print_free_t ?opts x =
  Pervasives.print_endline (gen_free_t x `piq ?opts)
let prerr_free_t ?opts x =
  Pervasives.prerr_endline (gen_free_t x `piq ?opts)

let print_memcpy_t ?opts x =
  Pervasives.print_endline (gen_memcpy_t x `piq ?opts)
let prerr_memcpy_t ?opts x =
  Pervasives.prerr_endline (gen_memcpy_t x `piq ?opts)

let print_memset_t ?opts x =
  Pervasives.print_endline (gen_memset_t x `piq ?opts)
let prerr_memset_t ?opts x =
  Pervasives.prerr_endline (gen_memset_t x `piq ?opts)

let print_fgetc_t ?opts x =
  Pervasives.print_endline (gen_fgetc_t x `piq ?opts)
let prerr_fgetc_t ?opts x =
  Pervasives.prerr_endline (gen_fgetc_t x `piq ?opts)

let print_lseek_t ?opts x =
  Pervasives.print_endline (gen_lseek_t x `piq ?opts)
let prerr_lseek_t ?opts x =
  Pervasives.prerr_endline (gen_lseek_t x `piq ?opts)

let print_lib_ident ?opts x =
  Pervasives.print_endline (gen_lib_ident x `piq ?opts)
let prerr_lib_ident ?opts x =
  Pervasives.prerr_endline (gen_lib_ident x `piq ?opts)

let print_fscanf_enum ?opts x =
  Pervasives.print_endline (gen_fscanf_enum x `piq ?opts)
let prerr_fscanf_enum ?opts x =
  Pervasives.prerr_endline (gen_fscanf_enum x `piq ?opts)
