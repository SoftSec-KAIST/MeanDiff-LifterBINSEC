...
let print_bitvector ?opts x =
  Pervasives.print_endline (gen_bitvector x `piq ?opts)
let prerr_bitvector ?opts x =
  Pervasives.prerr_endline (gen_bitvector x `piq ?opts)

let print_dbacodeaddress ?opts x =
  Pervasives.print_endline (gen_dbacodeaddress x `piq ?opts)
let prerr_dbacodeaddress ?opts x =
  Pervasives.prerr_endline (gen_dbacodeaddress x `piq ?opts)

let print_codeaddress ?opts x =
  Pervasives.print_endline (gen_codeaddress x `piq ?opts)
let prerr_codeaddress ?opts x =
  Pervasives.prerr_endline (gen_codeaddress x `piq ?opts)

let print_codeaddress_typeid_codeaddress ?opts x =
  Pervasives.print_endline (gen_codeaddress_typeid_codeaddress x `piq ?opts)
let prerr_codeaddress_typeid_codeaddress ?opts x =
  Pervasives.prerr_endline (gen_codeaddress_typeid_codeaddress x `piq ?opts)

let print_dbatag ?opts x =
  Pervasives.print_endline (gen_dbatag x `piq ?opts)
let prerr_dbatag ?opts x =
  Pervasives.prerr_endline (gen_dbatag x `piq ?opts)

let print_dbatag_typeid_dbatag ?opts x =
  Pervasives.print_endline (gen_dbatag_typeid_dbatag x `piq ?opts)
let prerr_dbatag_typeid_dbatag ?opts x =
  Pervasives.prerr_endline (gen_dbatag_typeid_dbatag x `piq ?opts)

let print_dbastopstate ?opts x =
  Pervasives.print_endline (gen_dbastopstate x `piq ?opts)
let prerr_dbastopstate ?opts x =
  Pervasives.prerr_endline (gen_dbastopstate x `piq ?opts)

let print_dbastopstate_typeid_dbastate ?opts x =
  Pervasives.print_endline (gen_dbastopstate_typeid_dbastate x `piq ?opts)
let prerr_dbastopstate_typeid_dbastate ?opts x =
  Pervasives.prerr_endline (gen_dbastopstate_typeid_dbastate x `piq ?opts)

let print_dbaexpr ?opts x =
  Pervasives.print_endline (gen_dbaexpr x `piq ?opts)
let prerr_dbaexpr ?opts x =
  Pervasives.prerr_endline (gen_dbaexpr x `piq ?opts)

let print_dbaexpr_typeid_dbaexpr ?opts x =
  Pervasives.print_endline (gen_dbaexpr_typeid_dbaexpr x `piq ?opts)
let prerr_dbaexpr_typeid_dbaexpr ?opts x =
  Pervasives.prerr_endline (gen_dbaexpr_typeid_dbaexpr x `piq ?opts)

let print_dbaexpr_dbaunary ?opts x =
  Pervasives.print_endline (gen_dbaexpr_dbaunary x `piq ?opts)
let prerr_dbaexpr_dbaunary ?opts x =
  Pervasives.prerr_endline (gen_dbaexpr_dbaunary x `piq ?opts)

let print_dbaexpr_dbabinary ?opts x =
  Pervasives.print_endline (gen_dbaexpr_dbabinary x `piq ?opts)
let prerr_dbaexpr_dbabinary ?opts x =
  Pervasives.prerr_endline (gen_dbaexpr_dbabinary x `piq ?opts)

let print_dbacond ?opts x =
  Pervasives.print_endline (gen_dbacond x `piq ?opts)
let prerr_dbacond ?opts x =
  Pervasives.prerr_endline (gen_dbacond x `piq ?opts)

let print_dbacond_typeid_dbacond ?opts x =
  Pervasives.print_endline (gen_dbacond_typeid_dbacond x `piq ?opts)
let prerr_dbacond_typeid_dbacond ?opts x =
  Pervasives.prerr_endline (gen_dbacond_typeid_dbacond x `piq ?opts)

let print_dba_lhs ?opts x =
  Pervasives.print_endline (gen_dba_lhs x `piq ?opts)
let prerr_dba_lhs ?opts x =
  Pervasives.prerr_endline (gen_dba_lhs x `piq ?opts)

let print_dba_lhs_typeid_dbalhs ?opts x =
  Pervasives.print_endline (gen_dba_lhs_typeid_dbalhs x `piq ?opts)
let prerr_dba_lhs_typeid_dbalhs ?opts x =
  Pervasives.prerr_endline (gen_dba_lhs_typeid_dbalhs x `piq ?opts)

let print_dbainstr ?opts x =
  Pervasives.print_endline (gen_dbainstr x `piq ?opts)
let prerr_dbainstr ?opts x =
  Pervasives.prerr_endline (gen_dbainstr x `piq ?opts)

let print_dbainstr_typeid_instrkind ?opts x =
  Pervasives.print_endline (gen_dbainstr_typeid_instrkind x `piq ?opts)
let prerr_dbainstr_typeid_instrkind ?opts x =
  Pervasives.prerr_endline (gen_dbainstr_typeid_instrkind x `piq ?opts)

let print_dba_list ?opts x =
  Pervasives.print_endline (gen_dba_list x `piq ?opts)
let prerr_dba_list ?opts x =
  Pervasives.prerr_endline (gen_dba_list x `piq ?opts)

let print_endianness ?opts x =
  Pervasives.print_endline (gen_endianness x `piq ?opts)
let prerr_endianness ?opts x =
  Pervasives.prerr_endline (gen_endianness x `piq ?opts)
