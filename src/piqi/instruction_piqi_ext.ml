...
let print_instr_pol ?opts x =
  Pervasives.print_endline (gen_instr_pol x `piq ?opts)
let prerr_instr_pol ?opts x =
  Pervasives.prerr_endline (gen_instr_pol x `piq ?opts)

let print_cpuid_pol ?opts x =
  Pervasives.print_endline (gen_cpuid_pol x `piq ?opts)
let prerr_cpuid_pol ?opts x =
  Pervasives.prerr_endline (gen_cpuid_pol x `piq ?opts)

let print_instr_ident ?opts x =
  Pervasives.print_endline (gen_instr_ident x `piq ?opts)
let prerr_instr_ident ?opts x =
  Pervasives.prerr_endline (gen_instr_ident x `piq ?opts)
