...
let print_message_start_exec ?opts x =
  Pervasives.print_endline (gen_message_start_exec x `piq ?opts)
let prerr_message_start_exec ?opts x =
  Pervasives.prerr_endline (gen_message_start_exec x `piq ?opts)

let print_message_bp_reached ?opts x =
  Pervasives.print_endline (gen_message_bp_reached x `piq ?opts)
let prerr_message_bp_reached ?opts x =
  Pervasives.prerr_endline (gen_message_bp_reached x `piq ?opts)

let print_message_patch ?opts x =
  Pervasives.print_endline (gen_message_patch x `piq ?opts)
let prerr_message_patch ?opts x =
  Pervasives.prerr_endline (gen_message_patch x `piq ?opts)

let print_message_infos ?opts x =
  Pervasives.print_endline (gen_message_infos x `piq ?opts)
let prerr_message_infos ?opts x =
  Pervasives.prerr_endline (gen_message_infos x `piq ?opts)

let print_message_decode_instr ?opts x =
  Pervasives.print_endline (gen_message_decode_instr x `piq ?opts)
let prerr_message_decode_instr ?opts x =
  Pervasives.prerr_endline (gen_message_decode_instr x `piq ?opts)

let print_message_decode_instr_instr_entry ?opts x =
  Pervasives.print_endline (gen_message_decode_instr_instr_entry x `piq ?opts)
let prerr_message_decode_instr_instr_entry ?opts x =
  Pervasives.prerr_endline (gen_message_decode_instr_instr_entry x `piq ?opts)

let print_message_decode_instr_bin_kind ?opts x =
  Pervasives.print_endline (gen_message_decode_instr_bin_kind x `piq ?opts)
let prerr_message_decode_instr_bin_kind ?opts x =
  Pervasives.prerr_endline (gen_message_decode_instr_bin_kind x `piq ?opts)

let print_message_decode_instr_reply ?opts x =
  Pervasives.print_endline (gen_message_decode_instr_reply x `piq ?opts)
let prerr_message_decode_instr_reply ?opts x =
  Pervasives.prerr_endline (gen_message_decode_instr_reply x `piq ?opts)

let print_message_decode_instr_reply_instr_entry ?opts x =
  Pervasives.print_endline (gen_message_decode_instr_reply_instr_entry x `piq ?opts)
let prerr_message_decode_instr_reply_instr_entry ?opts x =
  Pervasives.prerr_endline (gen_message_decode_instr_reply_instr_entry x `piq ?opts)

let print_message_start_symb_exec ?opts x =
  Pervasives.print_endline (gen_message_start_symb_exec x `piq ?opts)
let prerr_message_start_symb_exec ?opts x =
  Pervasives.prerr_endline (gen_message_start_symb_exec x `piq ?opts)

let print_message_start_symb_exec_trace_kind ?opts x =
  Pervasives.print_endline (gen_message_start_symb_exec_trace_kind x `piq ?opts)
let prerr_message_start_symb_exec_trace_kind ?opts x =
  Pervasives.prerr_endline (gen_message_start_symb_exec_trace_kind x `piq ?opts)

let print_message_output ?opts x =
  Pervasives.print_endline (gen_message_output x `piq ?opts)
let prerr_message_output ?opts x =
  Pervasives.prerr_endline (gen_message_output x `piq ?opts)

let print_message_output_output_kind ?opts x =
  Pervasives.print_endline (gen_message_output_output_kind x `piq ?opts)
let prerr_message_output_output_kind ?opts x =
  Pervasives.prerr_endline (gen_message_output_output_kind x `piq ?opts)
