...
let print_header_t ?opts x =
  Pervasives.print_endline (gen_header_t x `piq ?opts)
let prerr_header_t ?opts x =
  Pervasives.prerr_endline (gen_header_t x `piq ?opts)

let print_header_t_architecture_t ?opts x =
  Pervasives.print_endline (gen_header_t_architecture_t x `piq ?opts)
let prerr_header_t_architecture_t ?opts x =
  Pervasives.prerr_endline (gen_header_t_architecture_t x `piq ?opts)

let print_metadata_t ?opts x =
  Pervasives.print_endline (gen_metadata_t x `piq ?opts)
let prerr_metadata_t ?opts x =
  Pervasives.prerr_endline (gen_metadata_t x `piq ?opts)

let print_metadata_t_exception_t ?opts x =
  Pervasives.print_endline (gen_metadata_t_exception_t x `piq ?opts)
let prerr_metadata_t_exception_t ?opts x =
  Pervasives.prerr_endline (gen_metadata_t_exception_t x `piq ?opts)

let print_metadata_t_typeid_metadata_t ?opts x =
  Pervasives.print_endline (gen_metadata_t_typeid_metadata_t x `piq ?opts)
let prerr_metadata_t_typeid_metadata_t ?opts x =
  Pervasives.prerr_endline (gen_metadata_t_typeid_metadata_t x `piq ?opts)

let print_ins_con_info_t ?opts x =
  Pervasives.print_endline (gen_ins_con_info_t x `piq ?opts)
let prerr_ins_con_info_t ?opts x =
  Pervasives.prerr_endline (gen_ins_con_info_t x `piq ?opts)

let print_ins_con_info_t_typeid_con_info_t ?opts x =
  Pervasives.print_endline (gen_ins_con_info_t_typeid_con_info_t x `piq ?opts)
let prerr_ins_con_info_t_typeid_con_info_t ?opts x =
  Pervasives.prerr_endline (gen_ins_con_info_t_typeid_con_info_t x `piq ?opts)

let print_instruction_t ?opts x =
  Pervasives.print_endline (gen_instruction_t x `piq ?opts)
let prerr_instruction_t ?opts x =
  Pervasives.prerr_endline (gen_instruction_t x `piq ?opts)

let print_body_t ?opts x =
  Pervasives.print_endline (gen_body_t x `piq ?opts)
let prerr_body_t ?opts x =
  Pervasives.prerr_endline (gen_body_t x `piq ?opts)

let print_body_t_typeid_body_t ?opts x =
  Pervasives.print_endline (gen_body_t_typeid_body_t x `piq ?opts)
let prerr_body_t_typeid_body_t ?opts x =
  Pervasives.prerr_endline (gen_body_t_typeid_body_t x `piq ?opts)

let print_chunk_t ?opts x =
  Pervasives.print_endline (gen_chunk_t x `piq ?opts)
let prerr_chunk_t ?opts x =
  Pervasives.prerr_endline (gen_chunk_t x `piq ?opts)

let print_trace_t ?opts x =
  Pervasives.print_endline (gen_trace_t x `piq ?opts)
let prerr_trace_t ?opts x =
  Pervasives.prerr_endline (gen_trace_t x `piq ?opts)
